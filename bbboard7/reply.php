...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $replyName = htmlspecialchars($_POST["name"], ENT_QUOTES | ENT_HTML5);
    $replyText = htmlspecialchars($_POST["post"], ENT_QUOTES | ENT_HTML5);
    ...
    if ($nameLength > 14 || $textLength > 14000) {
        ...
        if (empty($errorMessage)) {
            try {
                ...
                // Update the timestamp of the original post to bring it to the top of the main board
                $updateStmt = $db->prepare("UPDATE posts SET updated_at = CURRENT_TIMESTAMP WHERE id = :parent_id AND parent_id IS NULL");
                $updateStmt->bindValue(':parent_id', $parentId, SQLITE3_INTEGER);
                $updateStmt->execute();
                ...
                $errorMessage = 'Failed to submit reply: ' . $e->getMessage();
            }
        }
    }
}
...
<div id="replyForm">
    <?php if (isset($errorMessage) && !empty($errorMessage)) { ?>
        <div class="error">
            <?php echo htmlspecialchars($errorMessage, ENT_QUOTES | ENT_HTML5); ?> <a href="reply.php?post_id=<?php echo $parentId; ?>">Return to reply mode</a>
        </div>
    <?php } ?>
    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?post_id=' . $parentId, ENT_QUOTES | ENT_HTML5); ?>" method="post" enctype="multipart/form-data">
        <table>
            <tr>
                <td>Name:</td>
...
<div class="reply">
    ...
    <?php if (!empty($parentPost['image'])) { ?>
        <img src="<?php echo htmlspecialchars($parentPost['image'], ENT_QUOTES | ENT_HTML5); ?>" alt="Post Image" width="200" height="200" style="float:left; margin-right:10px;">
    <?php } ?>
    <span class="postName"><?php echo htmlspecialchars($parentPost["name"], ENT_QUOTES | ENT_HTML5); ?></span>
    <p class="postText" style="word-wrap: break-word; overflow-wrap: break-word;"><?php echo nl2br(htmlspecialchars($parentPost["text"], ENT_QUOTES | ENT_HTML5)); ?></p>
    ...
    <?php foreach ($replies as $reply) { ?>
        <div class="reply">
            <?php if (!empty($reply['image'])) { ?>
                <img src="<?php echo htmlspecialchars($reply['image'], ENT_QUOTES | ENT_HTML5); ?>" alt="Reply Image" width="100" height="100" style="float:left; margin-right:10px;">
            <?php } ?>
            <span class="postName"><?php echo htmlspecialchars($reply["name"], ENT_QUOTES | ENT_HTML5); ?></span>
            <p class="postText" style="word-wrap: break-word; overflow-wrap: break-word;"><?php echo nl2br(htmlspecialchars($reply["text"], ENT_QUOTES | ENT_HTML5)); ?></p>
            <div style="clear: both;"></div>
        </div>
    <?php } ?>
</div>

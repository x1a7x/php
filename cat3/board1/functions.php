...
function render_board_index($db, $results = null): void {
    ...
    if ($results instanceof SQLite3Result) {
        ...
        // null means no posts
        // Just show empty board (no threads)
    }
    ?>
...
function render_single_thread(SQLite3 $db, array $post): void {
    ...
    // Link to the static thread page in the threads directory
...
function render_thread_page(array $op, array $replies): void {
    // Note: Now threads are in /threads directory. We'll use relative links.
    // Return link should go up one directory: ../index.html
    // The form's action to reply: ../reply.php?thread_id=<id>
    // The "Return" link: ../index.html
    // CSS and JS are absolute from root so no problem.
    ...
    <div class="thread" id="thread_<?php echo $thread_id; ?>" data-board="<?php echo htmlspecialchars($GLOBALS['board_name'], ENT_QUOTES, 'UTF-8'); ?>">
        <?php
